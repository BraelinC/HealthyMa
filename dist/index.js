  ...
  "server/dbStorage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async createUser(userData) {
        ...
        const userId = `user_${timestamp2}_${randomSuffix}`;
        ...
        try {
          if (userData.email) {
            ...
            if (existingByEmail) {
              ...
              console.log("\u2705 Updated existing user by email:", user2.id);
              return user2;
            }
          }
          if (userData.id) {
            ...
            if (existing) {
              ...
              console.log("\u2705 Updated existing user by ID:", user2.id);
              return user2;
            }
          }
          ...
          console.log("\u{1F50D} Insert error:", error);
          if (error?.code === "23505") {
...
var loginSchema, registerSchema, JWT_SECRET, JWT_EXPIRES_IN, OLD_JWT_SECRETS;
var init_auth = __esm({
  "server/auth.ts"() {
    "use strict";
    init_storage();
...
var GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_CALLBACK_URL, isGoogleOAuthConfigured;
var init_googleAuth = __esm({
  "server/googleAuth.ts"() {
    ...
    GOOGLE_CALLBACK_URL = process.env.REPLIT_DEV_DOMAIN ? `https://${process.env.REPLIT_DEV_DOMAIN}/api/auth/google/callback` : "https://workspace-braelincarranz1.replit.app/api/auth/google/callback";
    isGoogleOAuthConfigured = GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET;
...
// server/profileValidator.ts
var profileValidator_exports = {};
__export(profileValidator_exports, {
  getDefaultGoalWeights: () => getDefaultGoalWeights,
  validateProfileForMealGeneration: () => validateProfileForMealGeneration
});
function validateProfileForMealGeneration(profile) {
  const missingFields = [];
  if (!profile) {
    return {
      isValid: false,
...
function getDefaultGoalWeights(primaryGoal) {
...
var init_profileValidator = __esm({
  "server/profileValidator.ts"() {
    "use strict";
  }
});
...
async function registerRoutes(app2) {
    ...
    app2.get(
      ...
      async (req, res) => {
        try {
          ...
          const { token, user: userWithoutPassword } = await handleGoogleCallback2(user);
  ...
  app2.get("/api/recipes/saved", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User ID not found" });
  ...
  app2.get("/api/recipes/generated", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User ID not found" });
  ...
  app2.delete("/api/recipes/:id/save", authenticateToken2, async (req, res) => {
    try {
      const recipeId = parseInt(req.params.id);
      if (isNaN(recipeId)) {
  ...
  app2.post("/api/meal-plans", authenticateToken2, async (req, res) => {
    try {
  ...
  app2.put("/api/meal-plans/:id", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
  ...
  app2.delete("/api/meal-plans/:id", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      const planId = Number(req.params.id);
      if (!userId) {
  ...
  app2.get("/api/meal-plans/:id/completions", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
  ...
  app2.post("/api/meal-plans/:id/completions/toggle", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      const mealPlanId = Number(req.params.id);
  ...
  app2.post("/api/meal-plans/:id/complete", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      const mealPlanId = Number(req.params.id);
  ...
  app2.post("/api/meal-plan/generate", authenticateToken2, async (req, res) => {
    ...
    try {
      ...
      if (true) {
        ...
        const goalWeights = userProfile?.goal_weights || getDefaultGoalWeights2(primaryGoal || userProfile?.primary_goal || "Eat Healthier");
        const filters = {
          numDays,
          mealsPerDay,
          cookTime,
          ...
          // Ensure we always have a primary goal
          ...
          goalWeights,
          weightBasedEnhanced: true,
          // Enable V2 weight-based system
          heroIngredients: []
          // Will be populated by V2 system
        };
...
`);
  ...
  app2.get("/api/profile", authenticateToken2, async (req, res) => {
    try {
  ...
  app2.post("/api/profile", authenticateToken2, async (req, res) => {
    try {
      const userId = await getDatabaseUserId(req);
      if (!userId) {
            ...
            Promise.resolve().then(() => (init_cultureCacheManager(), cultureCacheManager_exports)).then(async ({ getCachedCulturalCuisine: getCachedCulturalCuisine2 }) => {
              try {
                for (const culture of profileData.cultural_background || []) {
                  await getCachedCulturalCuisine2(userId, [culture]);
  ...
  app2.put("/api/profile", authenticateToken2, async (req, res) => {
    try {
      const userId = await getDatabaseUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.get("/api/profile/weight-based", authenticateToken2, async (req, res) => {
    try {
      const userId = await getDatabaseUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.post("/api/profile/weight-based", authenticateToken2, async (req, res) => {
    try {
      const userId = await getDatabaseUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.put("/api/profile/weight-based", authenticateToken2, async (req, res) => {
    try {
      const userId = await getDatabaseUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
      ...
      if (existingProfile) {
        ...
        console.log("\u{1F4BE} Final profileData being updated:", profileData);
  ...
  app2.post("/api/culture-parser", authenticateToken2, async (req, res) => {
    try {
      const { text: text2 } = req.body;
      if (!text2 || typeof text2 !== "string") {
        return res.status(400).json({ message: "Text input is required" });
  ...
  app2.get("/api/cultural-cuisine/:cuisine", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      ...
      const cuisineData = await getCulturalCuisineData2(userId, cuisine);
      if (!cuisineData) {
  ...
  app2.post("/api/cache-cultural-cuisines", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
      ...
      const cachePromises = culturalBackground.map(
        (cuisine) => getCulturalCuisineData2(userId, cuisine)
      );
  ...
  app2.get("/api/culture-cache-stats", authenticateToken2, async (req, res) => {
    try {
  ...
  app2.get("/api/achievements", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.post("/api/achievements/trigger", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.get("/api/achievements/:achievementId", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.post("/api/enhanced-meal-plan", authenticateToken2, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
