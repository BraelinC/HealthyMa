...
export async function getCulturalCuisineData(userId: number, cultureTag: string, options: { forceRefresh?: boolean } = {}): Promise<CulturalCuisineData | null> {
...
function invalidateCacheEntry(userId: number, cultureTag: string): void {
...
export function getCachedCuisines(userId: number): string[] {
  return Object.keys(culture_dish_cache[userId] || {});
}

export async function getCachedCulturalCuisine(userId: number, culturalBackground: string[], options: { useBatch?: boolean, forceRefresh?: boolean } = {}): Promise<{ [key: string]: CulturalCuisineData } | null> {
...
export function clearUserCache(userId: number): boolean {
